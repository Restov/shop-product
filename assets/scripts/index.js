$(document).ready(function () {
  $("body").on(
    "click",
    ".product__buttons-number-minus, .product__buttons-number-plus",
    function () {
      let $row = $(this).siblings("input");
      let step = parseInt($row.attr("step"));
      let val = parseInt($row.val());
      $(this).hasClass("product__buttons-number-minus")
        ? (val -= step)
        : (val += step);
      $row.val(val);
      $row.change();
  ...
  $("body").on("click", ".product__button--add", function () {
    ...
    let tov = "товар";
    if ($input.val() > 1 && $input.val() < 5) tov = "товара";
    else if ($input.val() > 4) tov = "товаров";

    let text = `В корзину добавлен${
      $input.val() === "1" ? "" : "о"
    } ${$input.val()} ${tov}`;
    $.notify(text, "info");
  });
